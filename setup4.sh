...
echo
echo -e "${GREEN} This script will install and configure${NC} Unbound DNS ${GREEN}and, optionaly,${NC} Pi-Hole ${GREEN}add blocker${NC}"
echo
echo
...
if ! sudo apt -y install unbound; then
    echo -e "${RED} Failed to install packages. Exiting.${NC}"
    exit 1
fi
...
# Fetch the root hints file and update /usr/share/dns/root.hints
if wget https://www.internic.net/domain/named.root -qO- | sudo tee /usr/share/dns/root.hints; echo; then
    echo
    echo -e "${GREEN} Root hints file updated successfully.${NC}"
else
    echo
    echo -e "${RED} Failed to update the root hints file. Please check your network connection and permissions.${NC}"
fi
...
EOF
                ...
                echo
                echo -e "${YELLOW} Important: Confirm >>${NC} Update > OK ${YELLOW}<< on:${NC}" Existing Install Detected!
